...
- Summary
	...
	<img width="779" alt="new" src="https://user-images.githubusercontent.com/76843676/129188373-33be9e87-6111-4921-9d7d-7b57961a0d0a.PNG"><br/>
  	...
  	-수치평균, 단순회귀, gbm 세 방법으로 앙상블 했을 때 그 중 단순회귀가 가장 사용자별로 추천 결과가 의미있어 채택하였다.<br/>
            ...
            - code 파일명 : Collaborative_Filtering.ipynb 
	    - 생성된 데이터 : result_mean.csv, result_regression.csv, result_gbm.csv 사용한 데이터 : ratings.csv, contents.csv
...
- Summary
	...
	<p>(1)고려대학교 학생들을 대상으로 현재 교내 동아리 만족도에 대한 설문조사 수행 (N=50)</br>
    	- 해당 플랫폼이 필요하다 -> 51%, 매우필요하다 -> 26.5%<br/> 
  	- 실제로 해당 플랫폼을 사용할 것이다 -> 49% , 매우 그렇다 -> 38%  </p>
    
	<p>(2)기대효과 <br/>
    	- 해당 플랫폼이 수강 신청 전에 과목에 대한 정보를 얻는 사이트인 KLUE처럼, 동아리 선택을 위해 고려대학교 학생들에게 널리 쓰이는 유용한 사이트로 발전할 가능성이 있는가 -> 79.6% 그렇다고 응답  <br/>
    	- 동아리 지원 시기마다 고려대학교 교내 동아리 지원 및 선발에 도움을 줄 수 있는 플랫폼으로 발전할 가능성이 있는가 -> 87.7% 그렇다고 응답
  <br/>
 

 - 향후과제 
 1. 애기능 동아리 연합회, 각 단과대학 소속 동아리들까지 서비스 확장
 2. 동아리 게시판 만들기
 3. 이달의 동아리, 실시간 인기 동아리, 동아리 행사 소개 등 다양한 컨텐츠 확장시키기
